...
from pde.utils.layers import ConvLSTMFNOCell, SpectralConv2d


class ConvLSTMFNO_Model(nn.Module):
    r"""ConvLSTM Model
    ...
    def __init__(self, num_layers, num_hidden, configs, **kwargs):
        super(ConvLSTMFNO_Model, self).__init__()
        ...
        for i in range(num_layers):
            ...
            cell_list.append(
                ConvLSTMFNOCell(in_channel, num_hidden[i], height, width, self.configs['filter_size'],
                                       self.configs['stride'], self.configs['layer_norm'])
            )
        self.cell_list = nn.ModuleList(cell_list)
        self.conv_last = SpectralConv2d(num_hidden[num_layers - 1], self.frame_channel, [10,10])
