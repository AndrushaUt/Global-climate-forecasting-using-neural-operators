...
def step(model, train_data, x_t_minus_1, y_t_plus_1, pde_function_of_loss, regression_function_of_loss, hour=1,
         coeff_of_pde=1e6, is_using_time_tensor=False):
    
    if is_using_time_tensor:
...
def iterate(model, x_from_loader, y_from_loader, pde_function_of_loss, regression_function_of_loss,
    ...
    for hour in range(time_prediction):
        if hour == 0:
            ...
            x_t_minus_1, y_t_plus_1 = x_from_loader[:, 0], y_from_loader[:, hour]

        elif hour == 1:
            # prediction for the second hour
            x_t_minus_1, y_t_plus_1 = x_from_loader[:, 1], y_from_loader[:, hour]

        else:
            x_t_minus_1, y_t_plus_1 = y_from_loader[:, hour - 2], y_from_loader[:, hour]
...
def train(device, model, optimizer, scheduler, trainloader, testloader, dataset_std, dataset_mean,
    ...
    for epoch_number in range(epochs):
        ...
        test_regression_full = test_regression_full / len(testloader)
        test_pred_list.append(test_regression_full)

        test_pde_full = test_pde_full / len(testloader)
        ...
        if is_plotting_prediction:
            with torch.no_grad():
                prediction_to_plot_first_hour = model(x_test)
                test_data = torch.cat((x_test[:, 1].unsqueeze(1), prediction_to_plot_first_hour.unsqueeze(1)), dim=1)
                prediction_to_plot_second_hour = model(test_data)
